...
const TableView = ({ dataSource, loading, pagination, onChange, showNewFeature }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <div>
          {(record.parent !== null && !record.parent.isLead) && (<div className="parent-caption"><span>Parent:</span> <Link to={`/hierarchy/${record.id}`}>{record.parent.title}</Link><br /></div>)/* eslint-disable-line */}
          {(record.parent !== null && record.parent.isLead) && (<div className="parent-caption"><span>Lead:</span> <Link to={`/programs/${record.id}`}>{record.parent.title}</Link><br /></div>)/* eslint-disable-line */}
          ...
          {(record.useProjectRoles && showNewFeature) && [
            <Tooltip placement="right" overlayClassName="member-access-tooltip" title={<span>{record.editable === false && <i>You cannot access this project.<br /></i>}<i>Only these members can access: </i><br /><div className="divider" />{record.roles.map(role => <span><b>{role.name}</b> | <i>{role.role}</i><br /></span>)}</span>}>
  ...
  return (
    <Table
      ...
      onChange={onChange}
      rowClassName={(record) => record.editable ? '' : 'inaccessible'}
    />
  )
}
