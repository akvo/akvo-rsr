...
const Reported = ({
  ...
  const handleOnDeleteUpdate = (update) => {
    Modal.confirm({
      ...
      content: 'You’ll lose this update after you click OK',
      onOk() {
    ...
    <FinalForm
      ...
      render={({ form }) => {
        ...
        return [
          ...
          >
            <Row style={{ marginBottom: 10 }} type="flex" justify="end" align="top">
              <Col md={3} xs={6}>
                {edit ? <Button onClick={() => setEdit(null)} block>{t('CANCEL')}</Button> : null}
              </Col>
              ...
              <Col md={{ span: 3, offset: 2 }} xs={6}>
                {(!disableInputs && !edit) ? <Button type="primary" onClick={() => setEdit(period.id)} block>{t('Edit')}</Button> : null}
                {(!disableInputs && edit) ? <Button type="danger" onClick={() => handleOnDeleteUpdate(init)} block>{t('DELETE')}</Button> : null}
              </Col>
            </Row>
            {
              edit
