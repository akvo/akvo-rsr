...
const isFiltering = (filtering) => Object.keys(filtering)
...
const handleOnFilteringDisaggregations = (filtering, disaggregations) => {
  ...
  const hasAnyFilter = isFiltering(filtering)
  return Object.keys(dsgGroups)
    ?.map((dsgKey) => {
      ...
      return Object.keys(groupTypes)
        ...
        ?.filter((v) => ((hasAnyFilter && v.value) || !hasAnyFilter))
        ?.sort((a, b) => a.value - b.value)
    })
    ?.flatMap((dg) => dg)
}
        ...
        ?.map((i) => {
          const fp = i
            ...
            ?.map((p) => {
              ...
              const disaggregationContributions = r?.fetched ? handleOnFilteringDisaggregations(filtering, disaggregations) : []
              return ({
                ...p,
                contributors: cb,
                disaggregations,
            ...
            ?.filter((p) => hasAnyFilter ? (p?.contributors?.length) : p)
          return ({
            ...i,
            periods: fp
          })
...
export const handleOnFilterResult = (r, filtering, search) => {
  if (r?.fetched && (isFiltering(filtering) || search)) {
  ...
  return ({
    ...
    periods: i?.periods?.map((p) => {
      ...
      if (fp) {
        ...
        const contribB = getAllContributors(p?.contributors)
          ?.map((cb) => {
