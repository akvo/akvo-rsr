...
from .test_utils import MemoryMonitoringTestCase, with_memory_monitoring_settings
...
class RSRMemoryMetricsBasicTest(MemoryMonitoringTestCase):
    ...
    def setUp(self):
        ...
        self.counter_patcher = patch(
            "akvo.rsr.memory_monitoring.prometheus_metrics.Counter"
        )
        self.gauge_patcher = patch(
            "akvo.rsr.memory_monitoring.prometheus_metrics.Gauge"
        )
        self.histogram_patcher = patch(
            "akvo.rsr.memory_monitoring.prometheus_metrics.Histogram"
        )
    ...
    def test_metrics_initialization(self):
        ...
        expected_metrics = [
            ...
            "deletion_tracker_size",
            "deletion_tracker_cleanup_count",
            ...
            "aggregation_memory_usage",
            "monitoring_info",
        ]

        for metric_name in expected_metrics:
            self.assertTrue(
                hasattr(self.metrics, metric_name), f"Missing metric: {metric_name}"
            )

    def test_record_request_memory(self):
        """Test request memory recording."""
        self.metrics.record_request_memory(
            memory_before_mb=50.0, memory_after_mb=55.0, peak_memory_mb=60.0
        )
    ...
    def test_update_memory_metrics(self):
        ...
        with patch(
            "akvo.rsr.memory_monitoring.prometheus_metrics.psutil"
        ) as mock_psutil:
    ...
    def test_update_model_instance_metrics(self):
        ...
        with patch(
            "akvo.rsr.memory_monitoring.prometheus_metrics.gc.get_objects"
        ) as mock_get_objects:
