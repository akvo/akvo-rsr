...
class CreatedBeforeOrAfterGivenDateTestCase(DeleteUntitledAndUnpublishedProjectsCommand):
    ...
    def test_keep_after(self):
        project = self.create_program(title='')
        self.delete_before_two_days_ago()
        self.assertTrue(Project.objects.filter(id=project.id).exists())

    def test_remove_before(self):
        project = self.create_project(title='', created_at=self.SEVEN_DAYS_AGO)
        self.delete_before_two_days_ago()
        self.assertFalse(Project.objects.filter(id=project.id).exists())

    def test_remove_unpublished(self):
        project = self.create_project(title='Test project', published=False, created_at=self.SEVEN_DAYS_AGO)
        self.delete_before_two_days_ago()
        self.assertFalse(Project.objects.filter(id=project.id).exists())
...
class HaveRelationshipTestCase(DeleteUntitledAndUnpublishedProjectsCommand):
    ...
    def setUp(self):
        super().setUp()
        self.project = self.create_project(title='', created_at=self.SEVEN_DAYS_AGO)
