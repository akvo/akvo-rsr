...
class CumulativeTestMixin:
    ...
    def setup_project(self, percentage=False, disaggregations=False, contributors=None):
        ...
        if disaggregations:
            builder.with_disaggregations({
                self.DISAGGREGATION_CATEGORY: [self.DISAGGREGATION_TYPE_1, self.DISAGGREGATION_TYPE_2]
            })
        if contributors:
...
class SingleUserCumulativeUnitUpdatesTestCase(BaseTestCase, CumulativeTestMixin):
    ...
    def setUp(self):
        ...
        self.project = self.setup_project(disaggregations=True)
...
class CumulativePercentageUpdatesTestCase(BaseTestCase, CumulativeTestMixin):
    ...
    def setUp(self):
        ...
        user1 = self.create_user('test1@akvo.org', 'password', is_admin=True)
        user2 = self.create_user('test2@akvo.org', 'password', is_admin=True)
        self.project = self.setup_project(percentage=True, disaggregations=True)
...
class CumulativeAggregationTestCase(BaseTestCase, CumulativeTestMixin):
    ...
    def setUp(self):
        ...
        user1 = self.create_user('test1@akvo.org', 'password', is_admin=True)
        user2 = self.create_user('test2@akvo.org', 'password', is_admin=True)
        self.lead_project = self.setup_project(disaggregations=True, contributors=[
            {'title': 'Contrib #1'},
            {'title': 'Contrib #2'}
        ])
