...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for comment in orm.ProjectComment.objects.all():
            comment._meta.get_field('last_modified_at').auto_now = False
            comment.created_at = comment.time
            comment.last_modified_at = comment.time
            comment.save()
            comment._meta.get_field('last_modified_at').auto_now = True

    def backwards(self, orm):
        "Write your backwards methods here."
        for comment in orm.ProjectComment.objects.all():
            comment.time = comment.last_modified_at
            comment.save()
