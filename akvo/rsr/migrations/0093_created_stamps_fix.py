...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        If that also fails, as a last resort, we try to copy a neighbouring timestamp
        """
        ...
        def find_closest_timestamp(id, model):
            """ Last resort. Find the timestamp of "nearest neighbour" object, and use that.
            """
            # if we don't find a neighbour within offset 3 we give up
            offsets = [-1, 1, -2, 2, -3, 3]
            for offset in offsets:
                try:
                    obj = model.objects.get(pk=id + offset)
                ...
                if obj.created_at is None:
                    try:
                        ...
                        try:
                            ...
                            timestamp = find_closest_timestamp(obj.id, model.south_model)
                            if timestamp:
