...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for project in orm['rsr.Project'].objects.all():
            # Check if project has a 'sync_owner' and use that IATI id if available
            if project.sync_owner:
                organisation = project.sync_owner
                for ps in orm['rsr.Partnership'].objects.filter(project=project, organisation=organisation):
                    if ps.iati_activity_id:
                        project.iati_activity_id = ps.iati_activity_id
                        try:
                            project.save()
                            break
                        except:
            ...
            # Otherwise, check if other support partners have an IATI id
            else:
                for ps in orm['rsr.Partnership'].objects.filter(project=project, partner_type='support'):
                    if ps.iati_activity_id:
                        project.iati_activity_id = ps.iati_activity_id
                        try:
                            project.save()
            ...
            if project.sync_owner:
                ...
                for ps in orm['rsr.Partnership'].objects.filter(project=project, organisation=organisation):
                    if project.iati_activity_id:
                        ps.iati_activity_id = project.iati_activity_id
                        try:
                            ps.save()
                        except:
                            pass
