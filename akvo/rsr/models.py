...
from akvo.rsr.fields import LatitudeField, LongitudeField, NullCharField, ValidCharField, ValidTextField
...
class Country(models.Model):
    ...
    name = ValidCharField(_(u'country name'), max_length=50, unique=True, db_index=True,)
    iso_code = ValidCharField(_(u'ISO 3166 code'), max_length=2, unique=True, db_index=True, choices=ISO_3166_COUNTRIES,)
    continent = ValidCharField(_(u'continent name'), max_length=20, db_index=True,)
    continent_code = ValidCharField(_(u'continent code'), max_length=2, db_index=True, choices=CONTINENTS)

#    name = ValidCharField(_(u'country name'), max_length=50,)
#    iso_code = ValidCharField(_(u'ISO 3166 code'), max_length=2,  choices=ISO_3166_COUNTRIES, null=True, blank=True,)
#    continent = ValidCharField(_(u'continent name'), max_length=20, choices=OLD_CONTINENTS, null=True, blank=True)
#    continent_code = ValidCharField(_(u'continent code'), max_length=2, choices=CONTINENTS, null=True, blank=True)
...
#    city = ValidCharField(_(u'city'), blank=True, max_length=255, help_text=_('(255 characters).'))
#    state = ValidCharField(_(u'state'), blank=True, max_length=255, help_text=_('(255 characters).'))
#    country = models.ForeignKey(Country, verbose_name=_(u'country'))
#    address_1 = ValidCharField(_(u'address 1'), max_length=255, blank=True, help_text=_('(255 characters).'))
#    address_2 = ValidCharField(_(u'address 2'), max_length=255, blank=True, help_text=_('(255 characters).'))
#    postcode = ValidCharField(_(u'postcode'), max_length=10, blank=True, help_text=_('(10 characters).'))
...
class PartnerType(models.Model):
    id = ValidCharField(max_length=8, primary_key=True, unique=True)
    label = ValidCharField(max_length=30, unique=True)
...
class Partnership(models.Model):
    ...
    partner_type = ValidCharField(_(u'partner type'), max_length=8, db_index=True, choices=PARTNER_TYPES,)
    funding_amount = models.DecimalField(
        _(u'funding amount'), max_digits=10, decimal_places=2,
        blank=True, null=True, db_index=True
    )
    partner_type_extra = ValidCharField(
        _(u'partner type extra'), max_length=30,
        blank=True, null=True, choices=PARTNER_TYPE_EXTRAS,
    )
    iati_activity_id = ValidCharField(_(u'IATI activity ID'), max_length=75, blank=True, null=True, db_index=True,)
    internal_id = ValidCharField(
...
class Organisation(TimestampsMixin, models.Model):
    ...
    organisation_type = ValidCharField(_(u'organisation type'), max_length=1, db_index=True, choices=ORG_TYPES)
    ...
    iati_org_id = ValidCharField(_(u'IATI organisation ID'), max_length=75, blank=True, null=True, db_index=True, unique=True)
    ...
    phone = ValidCharField(_(u'phone'), blank=True, max_length=20, help_text=_(u'(20 characters).'))
    mobile = ValidCharField(_(u'mobile'), blank=True, max_length=20, help_text=_(u'(20 characters).'))
    fax = ValidCharField(_(u'fax'), blank=True, max_length=20, help_text=_(u'(20 characters).'))
    contact_person = ValidCharField(
    ...
    description = ValidTextField(_(u'description'), blank=True, help_text=_(u'Describe your organisation.'))
    
    notes = ValidTextField(verbose_name=_("Notes and comments"), blank=True, default='')
...
class InternalOrganisationID(models.Model):
    ...
    #TODO: add index
    identifier = ValidCharField(max_length=200, verbose_name=u'internal ID of referenced organisation',)
...
class OrganisationAccount(models.Model):
    ...
    account_level = ValidCharField(u'account level', max_length=12, choices=ACCOUNT_LEVEL, default='free')
...
class FocusArea(models.Model):
    ...
    name = ValidCharField(u'focus area name', max_length=50, help_text=_(u'The name of the focus area. This will show as the title of the focus area project listing page. (30 characters).'))
    ...
    description = ValidTextField(u'description', max_length=500, help_text=_(u'Enter the text that will appear on the focus area project listing page. (500 characters).'))
...
class Benchmarkname(models.Model):
    name = ValidCharField(_(u'benchmark name'), max_length=80, help_text=_(u'Enter a name for the benchmark. (80 characters).'))
...
class MiniCMS(models.Model):
    ...
    label = ValidCharField(u'label', max_length=50, help_text=u'The label is used for identification only', )
    feature_box = ValidTextField(
...
class Project(TimestampsMixin, models.Model):
    ...
    title = ValidCharField(_(u'title'), max_length=45, db_index=True, help_text=_(u'A short descriptive title for your project (45 characters).'))
    subtitle = ValidCharField(_(u'subtitle'), max_length=75, help_text=_(u'A subtitle with more information on the project (75 characters).'))
    status = ValidCharField(_(u'status'), max_length=1, choices=STATUSES, db_index=True, default='N', help_text=_(u'Current project state.'))
    ...
    current_image_caption = ValidCharField(_(u'photo caption'), blank=True, max_length=50, help_text=_(u'Enter a caption for your project picture (50 characters).'))
    current_image_credit = ValidCharField(_(u'photo credit'), blank=True, max_length=50, help_text=_(u'Enter a credit for your project picture (50 characters).'))
    ...
    # goal_1 = ValidCharField(_('goal 1'), blank=True, max_length=60, help_text=_('(60 characters)'))
    # goal_2 = ValidCharField(_('goal 2'), blank=True, max_length=60)
    # goal_3 = ValidCharField(_('goal 3'), blank=True, max_length=60)
    # goal_4 = ValidCharField(_('goal 4'), blank=True, max_length=60)
    # goal_5 = ValidCharField(_('goal 5'), blank=True, max_length=60)
    ...
    project_plan = ValidTextField(_(u'project plan'), blank=True, help_text=_(u'Detailed information about the project and plans for implementing: the what, how, who and when. (unlimited).'))
    sustainability = ValidTextField(_(u'sustainability'), help_text=_(u'Describe plans for sustaining/maintaining results after implementation is complete (unlimited).'))
    ...
    # project meta info
    language = ValidCharField(max_length=2, choices=settings.LANGUAGES, default='en', help_text=u'The main language of the project')
    project_rating = models.IntegerField(_(u'project rating'), default=0)
    notes = ValidTextField(_(u'notes'), blank=True, default='', help_text=_(u'(Unlimited number of characters).'))

    # budget
    currency = ValidCharField(_(u'currency'), choices=CURRENCY_CHOICES, max_length=3, default='EUR')
...
class Goal(models.Model):
    ...
    text = ValidCharField(_(u'goal'), blank=True, max_length=100, help_text=_(u'(100 characters)'))
...
class BudgetItemLabel(models.Model):
    label = ValidCharField(_(u'label'), max_length=20, unique=True, db_index=True)
...
class PublishingStatus(models.Model):
    ...
    status = ValidCharField(max_length=30, choices=PUBLISHING_STATUS, default='unpublished')
...
class Link(models.Model):
    ...
    kind = ValidCharField(_(u'kind'), max_length=1, choices=LINK_KINDS)
    url = models.URLField(_(u'URL'))
    caption = ValidCharField(_(u'caption'), max_length=50)
...
class UserProfile(models.Model, PermissionBase, WorkflowBase):
    ...
    phone_number = ValidCharField(max_length=50, blank=True)  # TODO: check uniqueness if non-empty
    validation = ValidCharField(_('validation code'), max_length=20, blank=True)
...
class ProjectUpdate(TimestampsMixin, models.Model):
    ...
    title = ValidCharField(_(u'title'), max_length=50, db_index=True, help_text=_(u'50 characters'))
    text = ValidTextField(_(u'text'), blank=True)
    language = ValidCharField(max_length=2, choices=settings.LANGUAGES, default='en', help_text=u'The language of the update')
    #status = ValidCharField(max_length=1, choices=STATUSES, default='N')
    ...
    photo_location = ValidCharField(_(u'photo location'), max_length=1, choices=PHOTO_LOCATIONS)
    photo_caption = ValidCharField(_(u'photo caption'), blank=True, max_length=75, help_text=_(u'75 characters'))
    photo_credit = ValidCharField(_(u'photo credit'), blank=True, max_length=25, help_text=_(u'25 characters'))
    video = models.URLField(_(u'video URL'), blank=True, help_text=_(u'Supported providers: Blip, Vimeo, YouTube'), verify_exists=False)
    video_caption = ValidCharField(_(u'video caption'), blank=True, max_length=75, help_text=_(u'75 characters'))
    video_credit = ValidCharField(_(u'video credit'), blank=True, max_length=25, help_text=_(u'25 characters'))
    update_method = ValidCharField(_(u'update method'), blank=True, max_length=1, choices=UPDATE_METHODS, db_index=True, default='W')
    ...
    user_agent = ValidCharField(_(u'user agent'), blank=True, max_length=200, default='')
    uuid = ValidCharField(_(u'uuid'), blank=True, max_length=40, default='', db_index=True,
...
class ProjectComment(models.Model):
    ...
    user = models.ForeignKey(User, verbose_name=_(u'user'))
    comment = ValidTextField(_(u'comment'))
...
class PaymentGateway(models.Model):
    name = ValidCharField(max_length=255, help_text=u'Use a short, descriptive name.')
    description = ValidTextField(blank=True)
    currency = ValidCharField(max_length=3, choices=CURRENCY_CHOICES, default='EUR')
...
class PayPalGateway(PaymentGateway):
    ...
    locale = ValidCharField(max_length=2, choices=PAYPAL_LOCALE_CHOICES, default='US')
...
class MollieGateway(PaymentGateway):
    partner_id = ValidCharField(max_length=10)
...
class Invoice(models.Model):
    ...
    engine = ValidCharField(u'payment engine', choices=PAYMENT_ENGINES, max_length=10, default='paypal')
    ...
    name = ValidCharField(max_length=75, blank=True, null=True)
    email = models.EmailField(blank=True, null=True)
    status = models.PositiveSmallIntegerField('status', choices=STATUS_CHOICES, default=1)
    http_referer = ValidCharField(u'HTTP referer', max_length=255, blank=True)
    campaign_code = ValidCharField(u'Campaign code', blank=True, max_length=15)
    is_anonymous = models.BooleanField(u'anonymous donation')
    # PayPal
    ipn = ValidCharField(u'PayPal IPN', blank=True, null=True, max_length=75)
    # Mollie
    bank = ValidCharField(u'mollie.nl bank ID', max_length=4, choices=get_mollie_banklist(), blank=True)
    transaction_id = ValidCharField(u'mollie.nl transaction ID', max_length=100, blank=True)
...
class PartnerSite(TimestampsMixin, models.Model):
    ...
    notes = ValidTextField(verbose_name=u'Akvo partner site notes', blank=True, default='')
    hostname = ValidCharField(_(u'hostname'), max_length=50, unique=True,
        help_text=_(
    ...
    custom_return_url_text = ValidCharField(_(u'Return URL text'), blank=True, max_length=50, default='',
        help_text=_(
    ...
    about_box = ValidTextField(_(u'about box text'), max_length=500, blank=True,
    ...
    default_language = ValidCharField(_(u'Site UI default language'),
                                        max_length=5,
    ...
    facebook_app_id = ValidCharField(_(u'Facebook App Id'), max_length=40, blank=True, null=True,
        help_text=_(
