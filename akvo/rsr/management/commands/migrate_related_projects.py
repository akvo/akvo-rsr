...
class Command(BaseCommand):
    ...
    def migrate_siblings(self):
        ...
        modified_projects = self.set_sibling_parents(sibling_groups)
    ...
    def set_sibling_parents(self, sibling_groups: List[List[Project]]) -> List[Project]:
        """
        ...
        for sibling_group in sibling_groups:
            ...
            if parent_count == 0:
                ...
                self.out(f"{parent_uuid} is the parent of {sibling_group}")
            else:
                self.out(f"{sibling_group} has multiple parents!")
