...
import { distinct, findChildrenFromCurrentState, parentModelName } from "../utils";
                    ...
                    objects: state[model].ids.reduce(
                        (acc, id) => {
                            ...
                            const children = findChildrenFromCurrentState(
                                state,
                                id,
                                c.CHILD_OBJECTS[model]
                            );
                            return {
                                ...acc,
                                [id]: {
                                    ...state[model].objects[id],
...
export default function modelsReducer(state = initialModels, action) {
    switch (action.type) {
        ...
        case c.UPDATE_MODEL_START: {
            ...
            const updatedState = update(modelState, {
                changing: { $set: true },
                changed: { $set: true }
            });
