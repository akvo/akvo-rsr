...
def crs_add(project):
    ...
    if crs:
        ...
        if not crs.loan_status_year and (crs.loan_status_currency or crs.loan_status_value_date or
                                         crs.interest_received or crs.principal_outstanding or
                                         crs.principal_arrears or crs.interest_arrears):
            all_checks_passed = False
        ...
        if not (crs.loan_status_currency or project.currency) and \
                (crs.loan_status_year or crs.loan_status_value_date or crs.interest_received or
                 crs.principal_outstanding or crs.principal_arrears or crs.interest_arrears):
