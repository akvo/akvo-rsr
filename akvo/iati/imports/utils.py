...
def add_log(iati_import, tag, error, project=None, severity=IatiImportLog.VALUE_NOT_SAVED):
    """
...
def get_text(element, version):
    ...
    if version[0] == '1':
        ...
        return element.findtext('narrative', default='')
...
class ImportHelper(object):
    ...
    The ImporterHelper is not used directly but is sub-classed for each tag or group of tags that we
    want to extract data from into one RSR model. The sub-class must implement the do_import()
    method where the actual importing takes place.
    """

    def __init__(self, iati_import, parent_elem, project, globals, related_obj=None):
        """

        :param iati_import: The IatiImport object for the current import
    ...
    def add_log(self, tag, field, error, severity=IatiImportLog.VALUE_NOT_SAVED):
        """
        Add a log entry in the IatiImportLog model.
        ...
        IatiImportLog.objects.create(
            ...
            text=u'{}\t{}\t{}\t{}'.format(tag, self.model, field, error),
