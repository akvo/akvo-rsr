...
def generate():
    ...
    for bundle in ASSET_BUNDLES:
        ...
        # 1.2 Persist to disk since the YUI compressor need a file
        bundle_file_path = '%s/../../mediaroot/%s%s.%s' % (cwd, path, bundle, ASSET_BUNDLES[bundle]['type'])
        ...
        try:
            mv_string = 'mv %s %s/../../mediaroot/%s%s_min_%s.%s' % (bundle_file_path, cwd, ASSET_BUNDLES[bundle]['path'], bundle, bundle_hash, ASSET_BUNDLES[bundle]['type'])
        ...
        # 1.5 Add the new file to the git index
        try:
            git_add_string = 'git add %s/../../mediaroot/%s%s_min_%s.%s' % (cwd, ASSET_BUNDLES[bundle]['path'], bundle, bundle_hash, ASSET_BUNDLES[bundle]['type'])
