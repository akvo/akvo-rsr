...
class RSRVirtualEnvConfigTest(mox.MoxTestBase):

    def setUp(self):
        super(RSRVirtualEnvConfigTest, self).setUp()
        ...
        self.virtualenv_config = RSRVirtualEnvConfig(self.deployment_host_config_values, self.codebase_config,
                                                     self.deployment_config, self.mock_time_stamp_formatter)

    def test_can_create_rsrvirtualenvconfig_instance(self):
        """fab.tests.config.rsr.virtualenv_config_test  Can create RSRVirtualEnvConfig instance"""

        self.assertIsInstance(RSRVirtualEnvConfig.create_instance(), RSRVirtualEnvConfig)

    def test_has_virtualenvs_home(self):
        """fab.tests.config.rsr.virtualenv_config_test  Has virtualenvs home"""

        self.assertEqual(self.expected_virtualenvs_home, self.virtualenv_config.virtualenvs_home)

    def test_has_rsr_virtualenv_name(self):
        """fab.tests.config.rsr.virtualenv_config_test  Has RSR virtualenv name"""

        self.assertEqual(self.expected_rsr_env_name, self.virtualenv_config.rsr_env_name)

    def test_has_rsr_virtualenv_path(self):
        """fab.tests.config.rsr.virtualenv_config_test  Has RSR virtualenv path"""

        expected_rsr_env_path = os.path.join(self.expected_virtualenvs_home, self.expected_rsr_env_name)

        self.assertEqual(expected_rsr_env_path, self.virtualenv_config.rsr_env_path)

    def test_has_rsr_requirements_file_path(self):
        """fab.tests.config.rsr.virtualenv_config_test  Has RSR requirements file path"""
        ...
        self.assertEqual(expected_rsr_requirements_file_path, self.virtualenv_config.rsr_requirements_path)

    def test_can_get_time_stamped_pip_install_log_file_path(self):
        """fab.tests.config.rsr.virtualenv_config_test  Can get time stamped pip install log file path"""
        ...
        self.assertEqual(expected_pip_install_log_file_path, self.virtualenv_config.time_stamped_pip_install_log_file_path())


def suite():
    return TestSuiteLoader().load_tests_from(RSRVirtualEnvConfigTest)
