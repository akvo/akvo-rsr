...
if [[ ! "${SKIP_BACKEND_TESTS:-}" = yes ]]; then
  ...
  log Running tests
  # docker-compose -p rsrci -f docker-compose.yaml -f docker-compose.ci.yaml exec web scripts/docker/dev/run-as-user.sh scripts/docker/ci/build.sh
fi
...
# # docker-compose -p rsrciprod -f docker-compose.yaml -f docker-compose.ci.yaml -f docker-compose.ci.prod.images.yaml exec web scripts/docker/dev/run-as-user.sh scripts/docker/ci/end-to-end.sh
# docker-compose -p rsrciprod -f docker-compose.ywml -f docker-compose.ci.yaml -f docker-compose.ci.prod.images.yaml exec puppeteer npm run test -- --ci
docker-compose -p rsrciprod -f docker-compose.ywml -f docker-compose.ci.yaml -f docker-compose.ci.prod.images.yaml exec puppeteer whoami
